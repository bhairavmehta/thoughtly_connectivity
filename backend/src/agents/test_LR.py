...
import os
import time
from tools.lightrag_tool import LightRAGTool, LightRAGToolConfig
...
if __name__ == "__main__":
    if os.path.exists(TEST_WORKING_DIR):
        print(f"Removing old test directory: {TEST_WORKING_DIR}")
        import shutil
        try:
            shutil.rmtree(TEST_WORKING_DIR)
        except OSError as e:
            print(f"Error removing directory {TEST_WORKING_DIR}: {e}")
